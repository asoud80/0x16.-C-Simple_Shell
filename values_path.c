...
/**
 * _values_path - Path In New Strings to be seperated
 *
...
int _values_path(char **arg, char **env)
{
	...
	if (stat(*arg, &stat_lineptr) == 0)
		return (-1);
	path_rela = _get_path(env);/** GET Content Of "PATH="*/
	if (!path_rela)
		return (-1);
	token = strtok(path_rela, ":"); /**tokenizes Content Of "PATH="*/
	len = _strlen(*arg); /**GET the Length Of Arguments*/
	while (token)
	{
